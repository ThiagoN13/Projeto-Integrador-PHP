...
class crudBus{  
	  ...
	  */   
	  ...
	  public function insertBus($placa, $lotacao_max, $num_onibus){
		 ...
		 if (!empty($placa) && !empty($lotacao_max) && !empty($num_onibus)){
		 	foreach ($dados as $key) {
		  			if($key->placa == $placa or $key->numero_onibus == $num_onibus){
		  				echo "<script>alert('Este onibus já foi cadastrado')</script>";   
				   		echo "<script>window.location.href ='../php/cadastros.php';</script>";
					}
			}
			try{   
	  ...
	  public function viewItinerario($id_linha, $id_ponto){
		   try{   
			  $sql = "SELECT * FROM itinerario WHERE id_linha=? and id_ponto=?";   
			  $stm = $this->pdo->prepare($sql); 
	  ...
	  public function insertBusInLine($id_onibus, $id_linha){
		  if (!empty($id_onibus) && !empty($id_linha) ){
			try{   
			   ...
			   $stm->execute();   
			   echo "<script>alert('Registro inserido com sucesso')</script>";   
			   echo "<script>window.location.href ='../php/links.php';</script>";
			  } catch(PDOException $erro){   
			   echo "<script>alert('Erro na linha: {$erro->getLine()}')</script>"; 
			}   
		  } 
